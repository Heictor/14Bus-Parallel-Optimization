...
%POPULAÇÃO INICIAL
    ...
    %Para população de ínicio definido
        ...
        pop = [pop; population]; %pop irá salvar todas as populações geradas ao longo da execução
        %pop = population;
        % Cria uma matriz NxM, sendo N = O tamanho da população, e M = Quantidade
...
for generation = 1:1:maximal_generation
    ...
    for k = 1:1:population_size
        ...
        %geração "i" atual
        generation_fitness(k,:) = feval( fitness_function, [ population(1:population_size,1:number_of_variables) ] );
    ...
    for child=1:2:child_number
        ...
        %i+100
        %mother = population(15,:);
        mother = population( ceil((length(population))),: );
        %father = population(14,:);
...
%     pop_fitness = [pop_fitness ; feval(fitness_function, [pop(1,i:i*14) pop(2,i:i*14)])];
    %pop = [pop; population];
%   % Variation population
%   mutation_population = population( 1 :population_size, :); % elites do not participate in mutation, so start from 2
%    
